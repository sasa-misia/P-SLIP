...
def define_study_area_from_shapefile(study_area_fold, study_area_filename, id_field, id_selection, clip_polygons=None):
    ...
    excluded_polygon = geom.Polygon()
    shapefile_path = os.path.join(study_area_fold, study_area_filename)
    mun_polygons, mun_names = load_shapefile_polygons(shapefile_path, field_name=id_field, sel_filter=id_selection)
    if clip_polygons:
        clip_union = ops.unary_union(clip_polygons)
        mun_polygons = intersect_polygons(mun_polygons, clip_union)
    study_area_polygon = ops.unary_union(mun_polygons)
    min_extremes, max_extremes = get_polygon_extremes(study_area_polygon)
    study_area_vars = {
        'mun_polygons': mun_polygons,
        'study_area_polygon': study_area_polygon,
        'study_area_polygon_clean': study_area_polygon,
        'study_area_polygon_excluded': excluded_polygon,
        'max_extremes': max_extremes,
        'min_extremes': min_extremes
    }
    return study_area_vars, mun_names

def define_study_area_from_rectangles(rectangle_polygons):
    """Define study area from rectangle polygons."""
    excluded_polygon = geom.Polygon()
    mun_polygons = rectangle_polygons
    mun_names = [f"Poly {i+1}" for i in range(len(rectangle_polygons))]
    study_area_polygon = ops.unary_union(mun_polygons)
    min_extremes, max_extremes = get_polygon_extremes(study_area_polygon)
    study_area_vars = {
        'mun_polygons': mun_polygons,
...
def main(base_dir=None):
    ...
    study_area_vars_file = os.path.join(fold_var, 'StudyAreaVariables.pkl')
